...
class ThemePrimeWebsiteSale(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0, max_price=0.0, ppg=False, **post):
        ...
        if theme_id and theme_id.name.startswith('theme_prime'):
            ...
            # Aplicar impuestos (ej. 18%) al filtro de búsqueda
            tax_multiplier = 1.18
            min_price_taxed = min_price / tax_multiplier
            max_price_taxed = max_price / tax_multiplier
            ...
            if request.website._get_dr_theme_config('json_shop_filters')['show_category_count']:
                domain = self._prepare_filters_domain(
                    search=fuzzy_search_term, attrib_values=attrib_values, pricelist=pricelist,
                    min_price=min_price_taxed, max_price=max_price_taxed, **post)
                response.qcontext.update(get_category_count=ProductTemplate._get_product_category_count(domain=domain))

            if request.website._get_dr_theme_config('json_shop_filters')['show_attribute_count'] or \
               request.website._get_dr_theme_config('json_shop_filters')['hide_extra_attrib_value']:
                domain = self._prepare_filters_domain(
                    search=fuzzy_search_term, category=category, pricelist=pricelist,
                    min_price=min_price_taxed, max_price=max_price_taxed, **post)
                response.qcontext.update(get_attrib_count=ProductTemplate._get_product_attrib_count(attrib_values, domain=domain))

                # Ratings
                domain = self._prepare_filters_domain(
                    search=fuzzy_search_term, attrib_values=attrib_values, category=category, pricelist=pricelist,
                    min_price=min_price_taxed, max_price=max_price_taxed, **{**post, 'rating': 'skip'})
        ...
        for res_product, product in zip(result, products):
            ...
            if 'public_categ_ids' in fields and product.public_categ_ids:
                ...
                res_product['category_info'] = {
                    ...
                    'is_cotizable': first_category.is_cotizable,
                }
            # brand
