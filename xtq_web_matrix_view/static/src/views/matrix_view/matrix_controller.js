...
import { MatrixConfirmationDialog } from "./matrix_confirmation_dialog";
import { SearchBar } from "@web/search/search_bar/search_bar";
import { useSearchBarToggler } from "@web/search/search_bar/search_bar_toggler";
import { session } from "@web/session";
import { MatrixCogMenu } from "./matrix_cog_menu";
...
export class MatrixController extends Component {
  ...
  setup() {
    ...
    this.editable =
      this.activeActions.edit && this.props.editable
        ? this.archInfo.editable
        : false;
    this.onOpenFormView = this.openRecord.bind(this);
    this.hasOpenFormViewButton = this.editable
      ? this.archInfo.openFormView
      : false;
    this.model = useState(
    ...
    useBus(
      ...
      "direct-export-data",
      this.onDirectExportData.bind(this)
    );
  }
  ...
  async createRecord({ group } = {}) {
    ...
    if (this.editable && !list.isGrouped) {
      if (!(list instanceof DynamicRecordList)) {
        throw new Error("Matrix should be a DynamicRecordList");
      }
  ...
  async onExportData() {
    const dialogProps = {
      context: this.props.context,
      defaultExportMatrix: this.defaultExportMatrix,
  ...
  async onDirectExportData() {
    await this.downloadExport(this.defaultExportMatrix, false, "xlsx");
  }
  /**
  ...
  onWillSaveMulti(editedRecord, changes, validSelectedRecords) {
    if (this.hasMousedownDiscard) {
      this.nextActionAfterMouseup = () =>
        this.model.root.multiSave(editedRecord);
      return false;
    }
    if (validSelectedRecords.length > 1) {
      ...
      return new Promise((resolve) => {
        ...
        this.dialogService.add(MatrixConfirmationDialog, dialogProps, {
          onClose: () => {
