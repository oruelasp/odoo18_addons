...
export class MatrixRenderer extends Component {
  static template = "xtq_web_matrix_view.MatrixRenderer";
  static rowsTemplate = "xtq_web_matrix_view.MatrixRenderer.Rows";
  static recordRowTemplate = "xtq_web_matrix_view.MatrixRenderer.RecordRow";
  static groupRowTemplate = "xtq_web_matrix_view.MatrixRenderer.GroupRow";
  ...
  setup() {
    ...
    useExternalListener(window, "keyup", async (ev) => {
      this.shiftKeyMode = ev.shiftKey;
    ...
    onPatched(async () => {
      ...
      await Promise.resolve();
      this.state.matrixData = this.matrixData;
  ...
  async loadMatrixData() {
    const filterBy = this.props.archInfo.xmlDoc.getAttribute("filter_by");
    const isFilterApplied =
      this.env.searchModel._domain
        .filter((rec) => Array.isArray(rec))
        .filter((rec) => rec[0].includes(filterBy)).length > 0;

    if (!isFilterApplied) {
      this.state.isChangedApplied = !this.state.isChangedApplied;
      this.matrixData = [];
      return;
    }

    const recordIds = this.props.list.records.map((rec) => rec._config.resId);
    this.matrixData = await rpc(
      `/web/dataset/call_kw/${this.props.list.resModel}`,
      {
        model: this.props.list.resModel,
        method: "get_matrix_data",
        args: [recordIds],
        kwargs: {},
      }
    );
  ...
  getFieldProps(record, column) {
    return {
      readonly:
        this.isCellReadonly(column, record) || this.isRecordReadonly(record),
    };
  }
  ...
  get canResequenceRows() {
    ...
    return (
      !orderBy.length || (orderBy.length && orderBy[0].name === handleField)
    );
  }
  ...
  getCellTitle(column, record) {
    ...
    if (
      ["many2one", "reference", "char"].includes(this.fields[column.name].type)
    ) {
  ...
  getLastAggregateIndex(group) {
    ...
    const index = reversedColumns.findIndex(
      (col) => col.name in group.aggregates
    );
  ...
  async onCellClicked(record, column, ev) {
    ...
    if (
      (this.props.list.model.multiEdit && record.selected) ||
      this.isInlineEditable(record)
    ) {
  ...
  findFocusFutureCell(cell, cellIsInGroupRow, direction) {
    ...
    switch (direction) {
      ...
      case "down": {
        ...
        futureRow =
          futureRow || row.parentElement.nextElementSibling?.firstElementChild;
        if (futureRow) {
          const addCell = [...futureRow.children].find((c) =>
            c.classList.contains("o_group_field_row_add")
          );
  ...
  async onCellChange(ev, group = null, record = null) {
    await rpc(`/web/dataset/call_kw/${this.props.list.resModel}/write`, {
      model: this.props.list.resModel,
      method: "write",
      args: [
        parseInt(ev.target.id),
        { [ev.target.getAttribute("field")]: parseFloat(ev.target.value) },
      ],
      kwargs: {},
    });
    await this.loadMatrixData();
  ...
  applyCellKeydownEditModeGroup(hotkey, _cell, group, record) {
    ...
    const isEnterBehavior =
      hotkey === "enter" && (!record.canBeAbandoned || isDirty);
  ...
  onCellKeydownEditMode(hotkey, cell, group, record) {
    ...
    if (
      group &&
      this.applyCellKeydownEditModeGroup(hotkey, cell, group, record)
    ) {
      return true;
    }
  ...
  get showNoContentHelper() {
    ...
    return (
      this.props.noContentHelp && (model.useSampleModel || !model.hasData())
    );
  }
